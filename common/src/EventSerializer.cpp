...
Event EventSerializer::createGameOverEvent(std::string eventString) {
    ...
    GameOverEvent gameOverEvent(GameOverEventType, playerId,highscores);
    ...
    for (int i = 0; i < numberOfUsers; ++i) {
        ...
        gameOverEvent.highscores[name] = {bullets, kills, score};
    }

    return Event(&gameOverEvent, GameOverEventType);
}
...
std::string EventSerializer::serialize(GameOverEvent& event) {
    ...
    std::string numberOfUsers = std::to_string(event.highscores.size());
    addZerosToLeft(numberOfUsers, 3);

    std::string response = GAME_OVER_EVENT_STRING + playerId + numberOfUsers;
    for (auto& userHighscore: event.highscores) {
